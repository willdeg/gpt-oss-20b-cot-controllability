#!/usr/bin/env python3
"""Run the balanced CoT-Control 60-question experiment on GPT-OSS-20B."""
...
CONDITIONS = [
    {
        "id": "sdf__enhanced_constraint",
        "model_condition": "sdf",
        "constraint_id": "enhanced_constraint",
        "constraint_text": ENHANCED_CONSTRAINT,
    },
    {
        "id": "baseline__enhanced_constraint",
        "model_condition": "baseline",
        "constraint_id": "enhanced_constraint",
        "constraint_text": ENHANCED_CONSTRAINT,
    },
    {
        "id": "baseline__no_constraint",
        "model_condition": "baseline",
        "constraint_id": "no_constraint",
        "constraint_text": "",
    },
    {
        "id": "baseline__original_constraint",
        "model_condition": "baseline",
        "constraint_id": "original_constraint",
        "constraint_text": ORIGINAL_CONSTRAINT,
    },
]
...
def main() -> None:
    ...
    if len(tasks) != 240:
        raise SystemExit(f"Expected exactly 240 tasks, found {len(tasks)}")
    ...
    logging.info("Experiment has %d tasks; SDF block is first", len(tasks))
    write_progress(args.experiment_dir, tasks, None, "running")
    run_block("sdf", tasks, args.experiment_dir, args.adapter_dir)
    run_block("baseline", tasks, args.experiment_dir, None)
