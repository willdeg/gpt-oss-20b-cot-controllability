...
ROOT_DIR="/workspace/selfbelief_v3"
PYTHON_BIN="$ROOT_DIR/venv/bin/python"
DATASET_PATH="$ROOT_DIR/data/synth_docs.jsonl"
TRAINER_PATH="$ROOT_DIR/scripts/finetune_gptoss_sdf_lora.py"
OUTPUT_DIR="$ROOT_DIR/output/self_belief_003_stronger_lora"
...

{
  ...
  "$PYTHON_BIN" -c 'import torch,transformers,peft,datasets,accelerate; print(f"torch={torch.__version__} transformers={transformers.__version__} peft={peft.__version__} datasets={datasets.__version__} accelerate={accelerate.__version__}")'
} > "$LOG_DIR/training_environment.txt"

set +e
"$PYTHON_BIN" "$TRAINER_PATH" \
